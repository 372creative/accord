...
import { CheckIcon, HeartIcon, InfoIcon } from './icons';

export function RecommendationCard({ rec }: { rec: Recommendation }) {
  const f = byId(rec.fragranceId);
  const { push, itemFor, setStatus } = useApp();
  ...
  return (
    <GlowCard glow>
      <div className="p-5">
        <div className="flex items-center gap-4" onClick={() => push({ type: 'fragrance', id: f.id })}>
